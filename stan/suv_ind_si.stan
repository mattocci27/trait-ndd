data {
  ...
  array[N] int<lower=0, upper=1> suv; // binary outcome (1 or 0)
  array[N] int<lower=1, upper=J> sp; // species identifier (integer)
  array[N] int<lower=1, upper=S> plot; // plot identifier (integer)
  array[N] int<lower=1, upper=P> station; // station identifier (integer)
  array[N] int<lower=1, upper=T> census; // census identifier (integer)
  array[N] int<lower=1, upper=M> tag; // individual seedling tag (integer)
}
