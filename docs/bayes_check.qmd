...
![](`r here::here("figs/beta_dry_traits.png")`)

```{r,eval=FALSE}
dry_trait_draws <- withr::with_dir(rprojroot::find_root('_targets.R'),
    targets::tar_read(fit_draws_logistic_simple_stan_data_dry_het_intrain_nlog))

dry_trait_mcmc <- withr::with_dir(rprojroot::find_root('_targets.R'),
    targets::tar_read(fit_mcmc_logistic_simple_stan_data_dry_het_intrain_nlog))

mcmc_stan_data <- dry_trait$data
...
trait <- mcmc_stan_data$u[sp_pred, ]

# scon
beta <- dry_trait_draws |>
  dplyr::select(contains(str_c("beta[", ind_pred))) |>
  as.matrix()
# scon
gamma <- dry_trait_draws |>
  dplyr::select(contains(str_c("gamma[", ind_pred))) |>
  as.matrix()

par_res <- NULL
for (i in 1:4000) {
  res <- beta[i, ] - gamma[i, ] %*% mcmc_stan_data$u
  res <- as.numeric(res)
...
gamma_m <- apply(tmp2, 2, median)
gamma_ll <- apply(tmp2, 2, quantile, 0.025)
gamma_hh <- apply(tmp2, 2, quantile, 0.975)
gamma_l <- apply(tmp2, 2, quantile, 0.25)
gamma_h <- apply(tmp2, 2, quantile, 0.75)
...
ggplot(pred_data, aes(x = trait))  +
  ...
  geom_point(data = res_data, aes(y = res_m), col = my_col[1]) +
...
- [There are no trait effects on CNDD from seedlings.](#without-rain)
