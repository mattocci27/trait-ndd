...
targets::tar_load(fit_11_dry_cn_int_s_summary_model_ind)
targets::tar_load(fit_12_wet_cn_int_s_summary_model_ind)
```
...
fit_11_dry_cn_int_s_summary_model_ind |> filter(rhat > 1.1)
fit_12_wet_cn_int_s_summary_model_ind |> filter(rhat > 1.1)
```
...
targets::tar_load(fit_11_dry_cn_int_s_diagnostics_model_ind)
targets::tar_load(fit_12_wet_cn_int_s_diagnostics_model_ind)
```
...
div_check(fit_11_dry_cn_int_s_diagnostics_model_ind)
div_check(fit_12_wet_cn_int_s_diagnostics_model_ind)
```
...
Traits include CN but not C and N.

```{r}
targets::tar_load(fit11_gamma)
fit11_gamma[2:6] <- round(fit11_gamma[2:6], 3)
```
```{r}
fit11_gamma |>
  filter(q2_5 * q97_5 > 0) |>
...
```{r}
targets::tar_load(fit12_gamma)
fit12_gamma[2:6] <- round(fit12_gamma[2:6], 3)
```
