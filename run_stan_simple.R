...
if (dry == "dry") {
  trait2 <- trait2 |>
    dplyr::select(sp, LDMC:C13, logLT)
} else {
  trait2 <- trait2 |>
    dplyr::select(sp, tlp, WD)
}
...
save_name <- str_c("./rda/", dry, "_spab_", n_ab, "_", model_name, "_", ng_data, "_", trait_data, "_", hab, "_simple.rda")
print(save_name)
