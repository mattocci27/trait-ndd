...
Models that did not converge well are shown in gray (N_ESS > 0 or N_Div $\geq$ 4).
...
```{r, include=FALSE}
dry_abund_gamma <- read_csv(here("data", "dry_phy_intrain_abund_gamma.csv"))
dry_abund_gamma |>
  filter(q2.5 * q97.5 > 0)
dry_abund_gamma |>
  filter(trait_name == "intercept")
...
```{r, include=FALSE}
wet_abund_gamma <- read_csv(here("data", "wet_phy_intrain_abund_gamma.csv"))
wet_abund_gamma |>
  filter(q2.5 * q97.5 > 0)
wet_abund_gamma |>
  filter(trait_name == "intercept")
