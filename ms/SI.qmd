...
The NUTS algorithm automatically determines the number of Hamiltonian dynamics simulation steps needed to propose a new state in the Markov chain without requiring the user to manually set a fixed number of steps [@Hoffman2014].
...
Four independent chains were run for 1,000 iterations for each model with a warm-up of 1,000 - 2,000 iterations.
...
The existence of divergent transitions suggest that estimates are based on the incomplete exploration of the simulated Hamiltonian trajectory and are not trusted [@Betancourt2016].
...
#   loo_list <- loo_list[!str_detect(names(loo_list),
#     "fit_mcmc_logistic_simple_stan_data_wet_het_norain_nlog")]
#   loo_list <- loo_list[!str_detect(names(loo_list),
#     "fit_mcmc_logistic_simple_stan_data_wet_phy_norain_nlog")]
#   loo_list <- loo_list[!str_detect(names(loo_list),
#     "fit_mcmc_logistic_simple_stan_data_wet_phy_rain_nlog")]
...
# tmp <- str_replace_all(tmp, "loo_fit2_mcmc", "fit2_summary")
tmp2 <- str_replace_all(loo_names, "loo_fit_mcmc", "fit_diagnostics")
# tmp2 <- str_replace_all(tmp2, "loo_fit2_mcmc", "fit2_diagnostics")
...
```{r, include=FALSE}
wet_traits_gamma2 <- read_csv(here("data", "wet_phy_intrain2_traits_gamma.csv"))
wet_traits_gamma2 |>
  filter(q2.5 * q97.5 > 0)
wet_traits_gamma2 |>
  filter(trait_name == "intercept")
...
```{r, include=FALSE}
dry_abund_gamma <- read_csv(here("data", "dry_het_intrain_abund_gamma.csv"))
dry_abund_gamma |>
  filter(q2.5 * q97.5 > 0)
dry_abund_gamma |>
  filter(trait_name == "intercept")
...
```{r, include=FALSE}
wet_abund_gamma <- read_csv(here("data", "wet_phy_rain_abund_gamma.csv"))
wet_abund_gamma |>
  filter(q2.5 * q97.5 > 0)
wet_abund_gamma |>
  filter(trait_name == "intercept")
