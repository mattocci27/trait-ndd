...
\(c\) = 0.24 for the dry season and \(c\) = 0.27 for the rainy season
were the best estimates for our dataset.
...
seedling heights, rainfall, densities of conspecific (\emph{ConS}) and
...
0.24 for the dry season or 0.27 for the rainy season, and the
interactions of rains with \emph{ConS}, with \emph{HetS}, with
\emph{ConA} and with \emph{HetA}.
...
(\(\boldsymbol{u_j}\)) includes LDMC, SDMC, RDMC, LA, SLA, Chl, LT,
\(\delta\)C\textsubscript{13}, C, N, CN, and \(\pi\)\textsubscript{tlp}
To allow comparisons among parameter estimates, all the predictor
variables (\(\boldsymbol{x_i}\) and \(\boldsymbol{u_j}\)) were scaled to
